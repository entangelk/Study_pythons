...
def solution(friends, gifts):
    answer = 0
    gifts_list_dict = {i:{j:0 for j in friends if j != i} for i in friends}
    get_gifts_dict = {i:{j:0 for j in friends if j != i} for i in friends}
    trans_gifts_dict = {i:{j:False for j in friends if j != i} for i in friends}


    for i in gifts:
        who,whom = map(str, i.split(' '))
        gifts_list_dict[who][whom] += 1
        get_gifts_dict[whom][who] += 1
        trans_gifts_dict[who][whom] = True

    for i in friends:
        counter = 0
        for j in friends:
            if i != j:
                if trans_gifts_dict[i][j]:
                    if gifts_list_dict[i][j] > gifts_list_dict[j][i]:
                        counter += 1
                    elif gifts_list_dict[i][j] == gifts_list_dict[j][i]:
                        check = 0
                        check_sec = 0
                        for key,k in gifts_list_dict[i].items():
                            check += k
                            pass
                        for ket,k in get_gifts_dict[i].items():
                            check -= k
                            pass
                        for key,k in get_gifts_dict[j].items():
                            check_sec -= k
                            pass
                        for ket,k in gifts_list_dict[j].items():
                            check_sec += k
                            pass                    
                        if check > check_sec:
                            counter += 1     
                        pass
                elif trans_gifts_dict[j][i]:
                    pass
                else:
                    check = 0
                    check_sec = 0

...
sum_list=[]
for i in range(9):
    set_board.append(list(map(int, input().split())))

for i in (set_board):
    temp_board.append(i)

for i in range(9):
    sorted_temp.append(sorted(temp_board[i], reverse=True))
    sum_list.append(sorted_temp[i][0])

sum_list.sort(reverse=True)

for i in range(9):
    try :
        result = set_board[i].index(sum_list[0])
        print(sum_list[0])
        print(i+1,result+1)
        break
    except:

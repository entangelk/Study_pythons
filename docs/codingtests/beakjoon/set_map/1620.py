...
import sys
def solution_make_data(count):
    data ={}
    data_reverse ={}
    for i in range(count):
        input_data = sys.stdin.readline().rstrip()
        data[input_data]= i+1
        data_reverse[i+1] = input_data
    return data, data_reverse

def solution_test_solve(dic_data,dic_reverse,num):
    answer_list = []
    for i in range(num):
        get_input = sys.stdin.readline().rstrip()
        answer_list.append(get_input)
...
get_num , test_num = map(int, sys.stdin.readline().split())

data,data_reverse = solution_make_data(get_num)
answer = solution_test_solve(data,data_reverse,test_num)